document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add the folder name on left
    const getAuxiliaryBarTitle = () => {
        const auxiliaryBarTitle = document.querySelector(".composite.title.has-composite-bar");
        return auxiliaryBarTitle;
    };

    const getFolderName = () => {
        const spanWithFolderName = document.querySelector("span.search-label");
        if (!spanWithFolderName) throw new Error('Folder name not found');

        return spanWithFolderName.textContent || '🗎';
    };
    const observer = new MutationObserver((mutationsList, observer) => {
        for (let mutation of mutationsList) {
            if (mutation.type === 'childList') {
                const auxiliaryBarTitle = getAuxiliaryBarTitle();
                if (auxiliaryBarTitle) {
                    const folderName = getFolderName();
                    auxiliaryBarTitle.innerHTML = `<span class="custom-folder-name">📂${folderName}</span>`;
                    observer.disconnect();
                    break;
                }
            }
